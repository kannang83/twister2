...
public class MeanCalculator {
  ...
  private static final Logger LOG = Logger.getLogger(MeanCalculator.class.getName());

  private double[] points;
  private double[][] centerSums;
  private double[][] centroids;

  private double[] meanSums;
  ...
  public MeanCalculator(double[] points, int taskId,
  ...
  public double calculate() {
    ...
    double meanValue = calculate(points);
    //MeanCenters meansCenters = new MeanCenters(centerSums);
    //return meansCenters;
    return meanValue;
  }

  private double calculate(double[] datapoints) {
    ...
    for (int i = startIndex; i < endIndex; i++) {
      //LOG.info("Datapoints:" + i + datapoints[i]);
      sum += datapoints[i];
      count++;
    }
    ...
    LOG.info("Mean value:" + meanValue + "\t" + startIndex + "\t" + endIndex);


    return meanValue;
  }

  public double calculateMean(double[] value1, int length) {
    double sum = 0;
